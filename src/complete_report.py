...
class CompleteReportGenerator:
    ...
    def generate_report_for_file(self, file_name: str, file_info: dict) -> str:
        ...
        try:
            ...
            pdf.add_page()
            self._add_cover_page_with_kpis(pdf, file_name, len(sheets_analysis), sheets_analysis, kpi_data)
            ...
            for sheet in sheets_analysis:
                if not sheet.get('is_kpi_sheet', False):
                    ...
                    # Skip sheets without charts entirely (no empty pages)
            
            # Final Summary Page (only if there are data sheets)
            data_sheets = [s for s in sheets_analysis if not s.get('is_kpi_sheet', False)]
            if data_sheets:
                pdf.add_page()
                self._add_final_summary(pdf, sheets_analysis, kpi_data)
    ...
    def _add_cover_page_with_kpis(self, pdf: FPDF, file_name: str, sheet_count: int, sheets_analysis: list, kpi_data: dict):
        """Add cover page with KPIs"""
        ...
        # pdf.cell(0, 6, f"Total Sheets: {sheet_count}  |  Total Records: {total_rows:,}", ln=True, align="C")
        pdf.ln(10)
        
        # KPIs Section (if available)
        if kpi_data and kpi_data.get('categories') and len(kpi_data['categories']) > 0:
            ...
            for category, metrics in categories.items():
                ...
                if pdf.get_y() > 200:
                    pdf.add_page()
                    pdf.set_font("Arial", "B", 14)
                    pdf.cell(0, 8, "Key Performance Indicators (continued)", ln=True, align="C")
                    pdf.ln(5)
                    start_y = pdf.get_y()
                ...
                pdf.set_text_color(255, 255, 255)
                pdf.cell(0, 7, f" {self._clean_text(category)}", ln=True, fill=True)
                pdf.set_text_color(0, 0, 0)
                pdf.ln(2)
                ...
                for i, metric in enumerate(metrics):
                    ...
                    pdf.cell(card_width - 6, 5, self._clean_text(metric['name'][:22]), ln=True, align='C')
                    
                    pdf.set_xy(x + 3, y + 14)
                    pdf.set_font("Arial", "B", 12)
                    pdf.set_text_color(metric['color'][0], metric['color'][1], metric['color'][2])
                    pdf.cell(card_width - 6, 8, self._clean_text(metric['value']), ln=True, align='C')
    ...
    def _add_data_sheet_report(self, pdf: FPDF, sheet: dict, temp_files: list):
        """Add data sheet report with charts - ONLY adds pages that have content"""
        ...
        if not has_any_chart:
            # Skip sheets without charts entirely (no page)
            return
        
        # First page: Sheet information
        pdf.add_page()
        ...
        pdf.set_text_color(0, 0, 0)
        pdf.set_font("Arial", "I", 9)
        pdf.cell(0, 5, f"Rows: {sheet['row_count']:,} | Columns: {sheet['column_count']}", ln=True)
        pdf.ln(3)
        
        if sheet.get('date_range'):
            pdf.set_font("Arial", "I", 8)
            pdf.set_text_color(100, 100, 100)
            pdf.cell(0, 5, f"Date Range: {sheet['date_range']['start']} to {sheet['date_range']['end']}", ln=True)
            pdf.ln(3)
        
        # Show column names
        numeric_cols = sheet.get('numeric_columns', [])
        categorical_cols = sheet.get('categorical_columns', [])
        
        pdf.set_font("Arial", "B", 9)
        pdf.cell(0, 5, "Data Overview:", ln=True)
        pdf.set_font("Arial", "", 8)
        if numeric_cols:
            pdf.cell(0, 4, f"  Numeric columns: {', '.join([self._clean_text(c[:15]) for c in numeric_cols[:5]])}", ln=True)
        if categorical_cols:
            pdf.cell(0, 4, f"  Categorical columns: {', '.join([self._clean_text(c[:15]) for c in categorical_cols[:5]])}", ln=True)
    ...
    def _add_final_summary(self, pdf: FPDF, sheets_analysis: list, kpi_data: dict):
        ...
        data_sheets = [s for s in sheets_analysis if not s.get('is_kpi_sheet', False)]
        kpi_sheets = [s for s in sheets_analysis if s.get('is_kpi_sheet', False)]
        
        pdf.cell(0, 6, f"Data Sheets: {len(data_sheets)}", ln=True)
        pdf.cell(0, 6, f"KPI Sheets: {len(kpi_sheets)}", ln=True)
        
        if kpi_data and kpi_data.get('all_metrics'):
            pdf.cell(0, 6, f"KPIs Extracted: {len(kpi_data['all_metrics'])}", ln=True)
        ...
        for sheet in sheets_analysis:
            if not sheet.get('is_kpi_sheet', False):
                pdf.cell(0, 4, f"  - {self._clean_text(sheet['sheet_name'])} (Data Sheet, {sheet['row_count']:,} rows)", ln=True)
